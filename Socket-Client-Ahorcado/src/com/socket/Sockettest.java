...
public class Sockettest extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			if (conectstatus) {// mostramos mensaje
				Sockettest.this.setVisible(false);
				Snd_txt_Msg("hola");
		...
		btconect.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (conectstatus) {// mostramos mensaje
					...
					Toast.makeText(getApplicationContext(),
							"Usted se ha conectado", Toast.LENGTH_SHORT).show();
					// juego j=new juego();
					...
					try {
						...
						// Manejamos flujo de Entrada
						ObjectInputStream ois = new ObjectInputStream(miCliente
								.getInputStream());
						// Cremos un Objeto con lo recibido del cliente
						Object aux = ois.readObject();// leemos objeto
	...
	/*
	 * public void Disconnect() { try { //Prepramos mensaje de desconexion
	 * Mensaje_data msgact = new Mensaje_data(); msgact.texto = "";
	 * msgact.Action = -1; msgact.last_msg = true; //avisamos al server que
	 * cierre el canal boolean val_acc = Snd_Msg(msgact);
	 * 
	 ...
	 * 
	 * Log.e("Disconnect() -> ", "!ok!"); //cerramos socket miCliente.close(); }
	...
	/*
	 * Metodo para enviar mensaje por socketrecibe como parmetro un objeto
	 * Mensaje_dataretorna boolean segun si se pudo establecer o no la conexion
	 */
